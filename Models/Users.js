...
const makeUser = async (
  ...
  let inital_nama =
    space != -1
      ? nama.toString().substring(0, 2)
      : nama.toString().substring(0, 1) +
        nama.toString().substring(space + 1, 1);

  let newKode = signature + "_" + inital_nama.toString().toUpperCase();
  let kode = await db.idMax(
    "kode",
    "mh_pelanggan",
    `WHERE kode LIKE '${newKode}%'`
  );
  newKode += kode;
  ...
  await db.executeQueryWithParam(query, [
    newKode,
    nama,
    email,
    password,
...
const makeLog = async (kodeuser, keterangan, total, jenis_transaksi) => {
  ...
  let query = `insert into log_transaksi (tgl_transaksi, kode_pelanggan, jenis_transaksi, nominal, keterangan) values('${date}','${kodeuser}','${jenis_transaksi}',${parseInt(
    total
  )},'${keterangan}')`;
  console.log(query);
  let data = await db.executeQuery(query);
